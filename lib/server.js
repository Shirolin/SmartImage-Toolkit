...
app.post('/api/split-custom', async (req, res) => {
    ...
    const filePath = path_1.default.resolve(rawPath);
    try {
        // 1. 执行切割
        ...
        if (result.status === 'success') {
            ...
            if (smartCenter) {
                ...
                for (const file of result.generatedFiles || []) {
                    ...
                    await (0, center_1.processCenter)(file, finalConfig, formatExt);
                }
            }
...
app.get('/api/open-file-dialog', (req, res) => {
    ...
    (0, child_process_1.exec)(cmd, { encoding: 'utf8' }, (error, stdout) => {
        if (error) {
