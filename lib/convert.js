...
(async () => {
    ...
    if (isInteractive) {
        ...
        if (resolution.trimConfig)
            trimConfig = resolution.trimConfig;
        if (resolution.cropConfig)
            cropConfig = resolution.cropConfig;
        if (resolution.centerConfig)
            centerConfig = resolution.centerConfig;
    }
    ...
    console.log(chalk_1.default.white(`📝 合计找到 ${chalk_1.default.cyan.bold(allFiles.length)} 个待处理文件，准备执行 ${targetFormat === 'split'
        ...
        : targetFormat === 'resize'
            ...
            : targetFormat === 'trim'
                ? chalk_1.default.yellow.bold(`[智能去边(Trim)]`)
                : targetFormat === 'crop'
                    ? chalk_1.default.yellow.bold(`[手动裁剪(Crop)]`)
                    : targetFormat === 'center'
                        ? chalk_1.default.magenta.bold(`[智能居中(Smart Center)]`)
    ...
    for (let i = 0; i < allFiles.length; i += batchSize) {
        ...
        const results = await Promise.all(batch.map(async (file) => {
            if (targetFormat === 'split' && splitConfig) {
                ...
                const res = await (0, trim_1.processTrimOrCrop)(file, 'trim', trimConfig, formatExt);
                return res;
            }
            else if (targetFormat === 'crop' && cropConfig) {
                const formatExt = cropConfig.outputFormat === 'original' ? null : `.${cropConfig.outputFormat}`;
                const res = await (0, trim_1.processTrimOrCrop)(file, 'crop', cropConfig, formatExt);
                return res;
            }
                ...
                const res = await (0, center_1.processCenter)(file, centerConfig, formatExt);
                return res;
            }
            else {
