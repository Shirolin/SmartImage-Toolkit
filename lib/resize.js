 ...
 */
async function resizeImage(filePath, options, formatExt) {
    ...
    try {
        ...
        switch (options.mode) {
            case 'by_width':
                if (!options.width)
                    return { status: 'error', file: filePath, reason: '按宽度缩放缺少宽度参数' };
            ...
            case 'by_height':
                if (!options.height)
                    return { status: 'error', file: filePath, reason: '按高度缩放缺少高度参数' };
