    ...
    while (true) {
        ...
        if (selectedFormat === 'trim') {
            ...
            if (trimMode === 'auto') {
                ...
                trimConfig = { threshold, sides: sides as ('top' | 'bottom' | 'left' | 'right')[] };
            } else {
                renderHeader('主界面 > 边缘修剪 (2/3) - 裁剪边界');
                console.log(
                    chalk.cyan(
                        '📏 请分别输入上、下、左、右四个方向需要向内切除的像素数值 (如果不需要切则输入 0 或直接敲回车):'
                    )
                );
            ...
            if (outFormat === 'back') continue;

            const finalFormat = outFormat as 'original' | 'webp' | 'png' | 'mozjpeg';
            console.clear();
